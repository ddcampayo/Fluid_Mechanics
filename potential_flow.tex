...
\S \ref{ss:Euler_energy}. Since the flow is incompressible, the rate in variation
of the specific energy is:
\[
...
\[
\frac{d \epsilon}{d t} =  \bfu \cdot \left( \frac12  \nabla u^2\right)  .
\]
But this is trivial, since $\epsilon = u^2 /2 $ plus constant terms (the internal
energy is constant along a streamline in incompressible ideal flow, \eqref{eq:Euler_energy}),
and in this steady state only the time derivative is just the convective derivative.
...
for potential flow around a cylinder, defined as the time variation
of the specific kinetic energy. We see there are two zones in the upwind face in which the
fluid speeds up, and two zones in the downwind face where it would slows down. The
...
\begin{figure}
	...
	\caption{Specific power (change in specific energy with time) for potential
		flow past cylinder. This is due only to changes in the kinetic energy. \label{fig:potential_flow_past_cylinder_energy}}
\end{figure}
