...
the typical velocity, \ref{eq:reduced_u0}, both are seen to coincide.
...
\[
  ...
  \frac{\partial^2  u_x}{\partial y^2}  + \cos(2\pi y) .
\]

The solution is easily found. First, the stationary state corresponds
to setting the time derivative to zero. In reduced units,
\[
\frac{\partial^2  u_x}{\partial y^2}  + \cos(2\pi y) = 0.
\]

The solution is then $u=\cos(2\pi y) /(2\pi)^2 $: a velocity profile
that is proportional to the applied force. Trying a solution that
includes this (homogeneous) solution, plus a (non-homogeneous) time-dependent
term, we arrive at
\[
...
Going back to dimensioned quantites, we may write
\[
...
It a simple exercise to arrive to this solution from the more general 
on of \eqref{eq:Fourier_simple_solution}, where pressure and convective acceleration
were assumed to be zero. See \cite{chatterjee_kolmogorov_2020} for an application of Fourier
techniques to the instability of this sort of flow. The idea is to incorporate a minimum number
of modes so that the solution may become unstable.


\section{General solution in the overdamped limit}
...
We recover the expressions of \eqref{eq:F_r_to_q} and \eqref{eq:F_q_to_r},
with a slight change of notation: we follow the convention that is usual in physics,
in which Fourier components are indicated by aa index --- also, no tilde (``wiggly'')
